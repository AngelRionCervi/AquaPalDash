<script lang="ts">
  ...
  const currentDevice = configStore.config?.devices.find((device) => device.id === childProps?.id);
  ...
  function getAvailableButtons() {
    const devicesButtonArray = [...new Array(MAX_DEVICES).fill(0).map((val, index) => (val += index))];

    const availableButtons = devicesButtonArray
      .filter((button) => !(configStore.config?.devices || []).map((device) => device.button).includes(button))
...
<div class="button-slot-setting-container">
  <div class="top">
    ...
    <div class="inner-container right">
      ...
      {:else}
        <Select id={selectId} name="select_{selectId}" label="New button:" values={availableButtons} hasBorders onchange={onButtonChange} />
      {/if}
    </div>
  </div>
  <div class="bottom">
    <ErrorField messages={errorMessage} />
    <PrimaryButton disabled={!!errorMessage || currentDevice?.button === newButtonSlot} onclick={onValidate} label="OK" />
  </div>
</div>
